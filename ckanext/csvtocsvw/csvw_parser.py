...
def simple_columns(columns: OrderedDict)-> list:
    col_dict=OrderedDict([(key,{"id": str(value[CSVW.name]), "info": {"label": str(value[CSVW.name])}}) for key, value in columns.items() if str(value[CSVW.name])!="GID"])
    for key, value in col_dict.items():
        if CSVW.format in columns[key].keys():
            col_dict[key]["type"]=DataStoreTypes.get(columns[key][CSVW.format],"text")
            col_dict[key]["pandas_type"]=PandasTypes.get(columns[key][CSVW.format],"object")
...
def parse_csv_from_url_to_list(csv_data, columns: dict, delimiter: str=',',skiprows:int =0, num_header_rows: int=2, encoding: str='utf-8') -> List[List]:
    ...
    table_data=table_data.map(lambda x: str(x).replace(',','.').replace(".", "", str(x).count(".")) if ',' in str(x) else str(x))
